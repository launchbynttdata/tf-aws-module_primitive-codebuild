...
# ECR Access Policy Document
resource "aws_iam_policy" "ecr_acesss_policy" {
  name   = "${var.project_name}-ecr-access"
  policy = data.aws_iam_policy_document.ecr_vpc_permissions[0].json
}

# ECR Access Policy Attachment
resource "aws_iam_policy_attachment" "codebuild_ecr_vpc_policy_attachment" {
  name       = "${var.project_name}-ecr-vpc-policy-attachment"
  roles      = ["aws_iam_role.codebuild_role.name"]
  policy_arn = aws_iam_policy.ecr_acesss_policy.arn
}
